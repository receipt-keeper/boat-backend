...
async def test_user_registration_facts_page_by_registered_at_and_id_cursor(
    ...
    async with user_session_factory() as session:
        ...
        first_page = await use_case.execute(ListUserRegistrationFactsQuery(batch_size=2))
        second_page = await use_case.execute(
            ListUserRegistrationFactsQuery(batch_size=2, cursor=first_page.next_cursor)
        )
...
async def test_user_registration_facts_return_empty_page_when_no_users(
    ...
    async with user_session_factory() as session:
        page = await _use_case(session).execute(ListUserRegistrationFactsQuery(batch_size=10))

    assert page.facts == ()
    assert page.next_cursor is None


async def test_user_registration_facts_keep_inclusive_lower_and_exclusive_upper_window_bounds(
    user_session_factory: async_sessionmaker[AsyncSession],
) -> None:
    lower_bound = datetime(2026, 6, 25, 9, tzinfo=UTC)
    upper_bound = datetime(2026, 7, 2, 9, tzinfo=UTC)
    async with user_session_factory() as session:
        await _seed_users(session, ((1, lower_bound), (2, upper_bound)))

        page = await _use_case(session).execute(
            ListUserRegistrationFactsQuery(
                batch_size=10,
                registered_after=lower_bound,
                registered_before=upper_bound,
            )
        )
