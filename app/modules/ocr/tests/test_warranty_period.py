...
def test_ocr_warranty_period_rejects_out_of_range_values(value: int) -> None:
    ...
    assert [(detail.field, detail.message) for detail in captured.value.details] == [
        (
            "period_months",
            "무상 AS 기간은 1개월 이상 60개월 이하여야 합니다.",
        )
    ]


def test_ocr_result_keeps_60_month_write_boundary() -> None:
    ...
    assert result.period_months.value == 60
    assert result.expires_on == date(2029, 1, 31)
