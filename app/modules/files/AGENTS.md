...
Files는 업로드 이미지 metadata, local/S3 object storage 접근, content streaming, delete safety check를 소유한다. mock contract가 아니라 production vertical slice다.
...
├── infrastructure/      # SQLAlchemy persistence + local/S3 object storage adapter
...
| S3 storage | `infrastructure/storage/s3.py` | 기존 `ObjectStorage` 포트 유지. boto3 동기 호출을 worker thread에서 실행한다. |
...
- File settings는 `Settings`에 둔다: `file_storage_backend`, `file_storage_root`, `s3_bucket`, `s3_region`, `s3_endpoint_url`, `s3_access_key_id`, `s3_secret_access_key`, `file_max_upload_bytes`, `file_max_upload_count`, `file_allowed_content_types`.
...
- `file_storage_backend=s3`는 `s3_bucket`과 `s3_region`이 필수이며, 정적 S3 자격 증명은 access key와 secret key를 함께 설정한다.
