...
- 모든 source 브랜치명은 NFC 정규형이어야 하며 제어 문자, 양방향 제어 문자, 공백 또는 Git ref 금지 문자를 포함할 수 없습니다.
- Riido에서 복사한 `<작업키>-<제목>` 형식의 브랜치도 허용합니다. 제목에는 한글, ASCII 영문·숫자, `-`, `_`, `.`만 사용할 수 있습니다.
- `dependabot/**` 브랜치는 `develop`으로 PR을 보냅니다.
- Riido stacked PR은 작업 브랜치끼리 연결할 수 있으며, 최하단 브랜치만 `develop`을 대상으로 합니다.
- `main`에는 `release/vX.Y.Z` 또는 `hotfix/*` 브랜치만 병합할 수 있습니다. `X`, `Y`, `Z`에는 ASCII 숫자를 사용하며, 병합 후 동일 저장소의 `main`을 `develop`에 역병합합니다.
